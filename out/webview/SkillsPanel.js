...
class SkillsPanel {
    ...
    async handleOpenExternal(url) {
        try {
            ...
            const errorMsg = error instanceof Error ? error.message : String(error);
            (0, UI_1.logToOutput)(`[Webview] Open external error: ${errorMsg}`);
            this.postMessage({
                type: 'openExternalResult',
                success: false,
                error: errorMsg
    ...
    async handleCheckForUpdates(skillId, skillName, githubUrl, localPath) {
        try {
            const storage = (0, SkillsStorageService_1.getStorageService)();
            const workspaceRoots = this.getWorkspaceRoots();
            const resolvedLocalPath = localPath
            ...
            const errorMsg = error instanceof Error ? error.message : String(error);
            (0, UI_1.logToOutput)(`[Webview] Check for updates error: ${errorMsg}`);
            this.postMessage({
                type: 'checkForUpdatesResult',
                skillId,
                success: false,
